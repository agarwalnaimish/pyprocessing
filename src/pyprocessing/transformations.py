...
def rotate(*args):
    ...
    if len(args) == 1:
        glRotatef(math.degrees(args[0]), 0, 0, 1)
    else:
        glRotatef(math.degrees(args[0]), args[1], args[2], args[3])
...
def camera(*args):
    ...
    if len(args) == 0:
        ...
        gluLookAt(width / 2.0, height / 2.0,
...
def perspective(*args):
    ...
    if len(args) == 0:
        ...
        gluPerspective(60, width * 1.0 / height, cameraZ / 100.0,
                       cameraZ * 10.0)
    else:
        assert (len(args) == 4)
