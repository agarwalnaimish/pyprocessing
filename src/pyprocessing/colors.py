...
__all__ = ['_getColor', 'color', 'red', 'green', 'blue', 'alpha', 'hue',
...
def _getColor(*color):
    ...
    if len(color) == 1:
        ...
        n = color[0]
        if not isinstance(n, float) and (
                    n & 0xff000000 != 0 or n > attrib.colorRange[0]):
...
def color(*args):
    ...
    return int(a * 255) << 24 | int(r * 255) << 16 | int(g * 255) << 8 | int(
        b * 255)
...
def blend_overlay(a, b):
    ...
    if (ar < 128):
        ...
        cr = (255 - (((255 - ar) * (255 - br)) >> 7))
    if (ag < 128):
        cg = ((ag * bg) >> 7)
    else:
        cg = (255 - (((255 - ag) * (255 - bg)) >> 7))
    if (ab < 128):
        cb = ((ab * bb) >> 7)
    else:
        cb = (255 - (((255 - ab) * (255 - bb)) >> 7))
    # alpha blend (this portion will always be the same)
    return (low(((a & ALPHA_MASK) >> 24) + f, 0xff) << 24 |
            (peg(ar + (((cr - ar) * f) >> 8)) << 16) |
            (peg(ag + (((cg - ag) * f) >> 8)) << 8) |
...
def blend_burn(a, b):
    ...
    if (br == 0):
        ...
        cr = 255 - peg(((255 - ar) << 8) / br);
    if (bg == 0):
        cg = 0
    else:
        cg = 255 - peg(((255 - ag) << 8) / bg);
    if (bb == 0):
        cb = 0
    else:
        cb = 255 - peg(((255 - ab) << 8) / bb);
    # alpha blend (this portion will always be the same)
    return (low(((a & ALPHA_MASK) >> 24) + f, 0xff) << 24 |
            (peg(ar + (((cr - ar) * f) >> 8)) << 16) |
            (peg(ag + (((cg - ag) * f) >> 8)) << 8) |
...
blendfunc = [blend_blend, blend_add_pin, blend_sub_pin, blend_darkest,
             blend_lightest,
             blend_difference, blend_exclusion, blend_multiply, blend_screen,
             blend_overlay,
