...
def blend(source, x, y, swidth, sheight, dx, dy, dwidth, dheight, mode):
    ...
    # DARKEST Mode
        ...
        red = numpy.bitwise_and(_mix(numpy.bitwise_and(a, 0xff0000), red, f),
                                0xff0000)
        green = numpy.right_shift(numpy.bitwise_and(b, 0xff00), 8)
        green = numpy.multiply(green, f)
        green = _low(numpy.bitwise_and(a, 0xff00), green)
        green = numpy.bitwise_and(_mix(numpy.bitwise_and(a, 0xff00), green, f),
                                  0xff00)
        blue = numpy.multiply(numpy.bitwise_and(b, 0xff), f)
        blue = numpy.right_shift(blue, 8)
...
def loadPixels():
    ...
    for i in range(width * height):
        screen.pixels[i] = current.pixels[i]
...
def get(*args):
    ...
    if len(args) in (0, 4):
        ...
        if w != width or h != height:
            return PImage(
                pyglet.image.get_buffer_manager().get_color_buffer()).get(x, y,
                                                                          w, h)
        else:
            return PImage(pyglet.image.get_buffer_manager().get_color_buffer())
    else:
        x, y = args
        return PImage(pyglet.image.get_buffer_manager().get_color_buffer()).get(
            x, y)
